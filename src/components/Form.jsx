...
const Form = ({ onView }) => {
  ...
  useEffect(() => {
    const getData = async () => {
      const response = await axios.get(
        "https://min-api.cryptocompare.com/data/price?fsym=USDT&tsyms=MXN,ARS,COP,USD,PEN"
      );
  ...
  return (
    <form className="min-w-[449px] shadow-[0_24px_64px_rgba(0,0,0,.06),0_2px_6px_rgba(0,0,0,.08)]  rounded-[16px] border-[1px] border-[#efefef]">
      ...
      <div className="pt-[38px] px-[31px] pb-[42px] w-full">
        ...
        <div
          className={`rounded-[12px] ${
            isFocused
              ? "border-[1px] border-[#004cff] shadow-[0_0_0_3.5px_rgba(32,95,236,.25)]"
              : "border-[1px] border-[#d8d8d8]"
        ...
        {/* SECOND INPUT */}
        <div
          className={`rounded-[12px] mt-[16px] ${
            isFocused2
              ? "border-[1px] border-[#004cff] shadow-[0_0_0_3.5px_rgba(32,95,236,.25)]"
              : "border-[1px] border-[#d8d8d8]"
          } py-[5px] px-[12px] flex items-center justify-between w-full`}
        >
          ...
          >
            ...
            {openChange && (
              <div className="absolute top-[120%] right-0 bg-white flex flex-col shadow-xl pt-6 rounded-md w-[234px] h-[276px]">
                {data.map((el) => {
                  return (
                    <div
                      key={el.name}
                      className="flex gap-4 p-3 hover:bg-[#f9f9f9]"
                      onClick={() => {
                        changeInput(el);
                      }}
                    >
        ...
        >
          <div className="px-[8px] pt-[4px]">
            <div className="text-[14px]">Hisob raqami</div>
            <input
              maxLength={16}
        ...
        <button
          className="w-full text-[18px] bg-main text-white py-[18px] rounded-[36px] mt-[46px] hover:text-[#9e9e9e]"
          type="button"
          onClick={submitForm}
        >
          <h1>Sotib olish USDT</h1>
        </button>
      </div>
    </form>
  );
